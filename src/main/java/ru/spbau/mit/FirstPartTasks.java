...
public final class FirstPartTasks {
  ...
  public static List<Album> sortedFavorites(Stream<Album> s) {
    return s
      .filter(a -> a.getTracks().stream().mapToInt(Track::getRating).anyMatch(i -> i > 95))
  ...
  public static int moduloProduction(IntStream stream, int mod) {
    return stream.reduce(1, (acc, num) -> (acc * num % mod) % mod);
  }
